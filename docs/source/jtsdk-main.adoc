...
== Overview
* *{page}* brings a number of significant changes, the majority
of which are focused on {wsjtx}, {wsjt} and {wspr} command line option
enhancements.
...
compiling. This is of particular interest to low resource systems such ARM based
SoC devices where memory and clock speeds can produce unexpected results if
maximum expansion is allowed.
...
[[CHANGE-LOG]]
== Change Log
Many behind the scenes updates have been made which only affect things like
version information, screen displays and the like. Those items are listed under
<<MINOR-CHANGES,Minor Changes>>. The <<MAJOR-CHANGES,Major Change>> items
are as follows:

[[MAJOR-CHANGES]]
.Major Changes
* {wsjtx} dynamic menu lists for GA Release and Development branches
* {wsjtx} built list generation via the Menu System or cli entry
* {wsjtx} built list timer settings via Menu System or cli entry
* {wsjtx} multi-core selection via the Menu System or cli entry
* {wsjtx} Cmake option selection via Menu system ( Custom Build )
* CPU selection from the Menu system or cli entry
* Application Version {plus} SVN Revision separation is now fully functional
* (9) user selectable features to customize build script behavior
* Option status list via Menu System or cli entry


[[MINOR-CHANGES]]
.Minor Changes
* Numerous main screen display and error message updates
* Most help screens have been consolidated into a single file
* On screen build messages were updated to reflect the user option selections
* Updated copyright date periods
...
to the {devmail}. To be useful, bug reports should include at least the
...
== New Dialog Menus
As listed in the <<CHANGE-LOG,Change Log>>, there are a number of new features
and functions that affect various build scenarios. Each of the
<<MAJOR-CHANGES,Major Changes>> will be explained in this section, with
script specific features being expanded in their own sections.
...
and (2), direct command line entry.

The Main Menu is the start point for all menu driven selections. {wsjt}, {wspr}
and Hamlib3 do not have Sub-Menus 
...
This menu requires that *kbd:[jtsdk -u]* has been run at least one time. It is
generated from pass:[^/branches/*] that are capable of being built with the
current QT5 package set.
...
==== WSJT-X GA / RC Menu
This menu requires that *kbd:[jtsdk -u]* has been run at least one time. It is
generated from pass:[^/tags/*] that are capable of being built with the
current QT5 package set.
...
This menu requires that *kbd:[jtsdk -u]* has been run at least one time. The
Superbuild lists are a combination of pass:[^/branches/*] and pass:[^/tags/*]
that are capable of being built with the current QT5 package set.
...
[[USER-OPTIONS]]
=== User Options
New in v{version} is the ability set various options which influence certain
elements within each build scripts. In total, there are (9) features that can
be set in either the Dialog Menu system or direct command line entry.
...
==== Options Status
This simply lists ll the available options and whether or not they are enabled.
It also includes CPU and Timer data.
...
==== Options Matrix
The following matrix lists the options, where they apply and their description.
...
it. You can select as many as you like either enabling or disabling.

See <<OPTION-MATRIX-DETAILS,Options Matrix Details>> for full descriptions.
...
will be made unless the repository does not exist, in which case a checkout
will be performed.
=====
...
==== Set Update Timer
This function sets up a simple timer to trigger WSJT-X Menus updates. Each time
*{program}* is stated, the timer is checked. The the ET reaches the specified
value, the update is triggered. You can set the timer using *kbd:[jtsdk -t]*

The default timer is set at ( 1440 minutes ) or once every day. Daily is
( recommended ). You can run the update any time you wish with *kbd:[jtsdk -u]*
...
Many of the Dialog Menu options use the same options that were enabled for
command line use. The following matrix outlines the current set of options, 
indexed by script.

The Dialog Menu systems covers the majority of what is available with the
exception of the command line switches themselves.
...
=== JTSDK Main Script
The *JTSDK* main script controls all of the user enable / disable options, as
well as {wsjtx} Menu generation, {wsjtx} Timer and user CPU settings. The
following list is all of the options available when using *kbd:[jtsdk -x]*,
*kbd:[  -x  ]* the option selection. The following matrix lists the available
commands.
...
== Installation
For Debian based distros (with the exception of Debian itself), its highly
...
*{page}*. Selecting minimal can save on a significant number pf packages, please
...
*{program}* before installing *v{version}* as many Makefile changes have been made.

There are two options you may want to consider; *kbd:[--disable-docs]* and / or
*kbd:[--disable-manpages]*. If have not installed *AsciiDoc*, *AsciiDoctor*
...
particular situation. You can find the list {sourcelist}.
